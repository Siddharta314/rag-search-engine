...
def create_parser():
    ...
    semantic_chunk_parser = subparsers.add_parser(
        "semantic_chunk", help="Semantic chunk a document"
    )
    ...
    semantic_chunk_parser.add_argument(
        "--overlap", type=int, default=0, nargs="?", help="Overlap between chunks"
    )

    semantic_chunk_parser = subparsers.add_parser(
        "search_chunked", help="Search in chunked documents by query"
    )
    semantic_chunk_parser.add_argument("query", help="Text to search")
    semantic_chunk_parser.add_argument(
        "--limit", type=int, default=5, nargs="?", help="Limit the number of results"
    )
